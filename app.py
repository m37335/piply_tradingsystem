...
def verify_github_webhook(request_data, signature):
    ...
    expected_signature = (
        "sha256="
        + hmac.new(
            GITHUB_WEBHOOK_SECRET.encode("utf-8"), request_data, hashlib.sha256
        ).hexdigest()
    )
...
def api_health_check():
    """APIヘルスチェックエンドポイント"""
    return jsonify({"status":"healthy","service":"exchanging-app","version":"1.0.0"})
...
def github_info():
    ...
    try:
        ...
        if repo_info:
            return jsonify(
                {"status": "ok", "repository": repo_info, "configured": True}
            )
        else:
            return (
                jsonify(
...
def manual_update():
    ...
    try:
        ...
        return jsonify(
            {"status": "accepted", "message": "Manual update process started"}
        )
