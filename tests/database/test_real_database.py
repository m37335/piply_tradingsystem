...
async def test_real_database():
    ...
    try:
        async with async_session() as db_session:
            ...
            result = await db_session.execute(
                text("SELECT DISTINCT indicator_type FROM technical_indicators ORDER BY indicator_type")
            )
            ...
            for indicator_type in indicator_types:
                result = await db_session.execute(
                    text("SELECT COUNT(*) FROM technical_indicators WHERE indicator_type = :type"),
