...
from scripts.cron.base_aggregator import BaseAggregator, AggregationError, InsufficientDataError
...
class TestBaseAggregator:
    ...
    def base_aggregator(self, mock_price_repo):
        ...
        with patch('scripts.cron.base_aggregator.PriceDataRepositoryImpl') as mock_repo_class:
    ...
    async def test_calculate_ohlcv(self, base_aggregator):
        ...
        test_data = [
            PriceDataModel(
                ...
                timestamp=datetime(2025, 1, 1, 10, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo")),
                open_price=100.0,
                high_price=105.0,
                low_price=99.0,
                close_price=102.0,
            ...
            PriceDataModel(
                currency_pair="USD/JPY",
                timestamp=datetime(2025, 1, 1, 10, 5, 0, tzinfo=pytz.timezone("Asia/Tokyo")),
                open_price=102.0,
                high_price=107.0,
                low_price=101.0,
                close_price=104.0,
        ...
        assert result.timestamp == datetime(2025, 1, 1, 10, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
    ...
    async def test_check_duplicate(self, base_aggregator, mock_price_repo):
        """重複チェックのテスト"""
        test_timestamp = datetime(2025, 1, 1, 10, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
...
class TestHourlyAggregator:
    ...
    async def test_get_aggregation_period(self, hourly_aggregator):
        ...
        with patch('scripts.cron.hourly_aggregator.datetime') as mock_datetime:
            ...
            mock_now = datetime(2025, 1, 1, 10, 30, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
            mock_datetime.now.return_value = mock_now

            start_time, end_time = await hourly_aggregator.get_aggregation_period()

            # 前1時間の期間（09:00-09:55）が返されることを確認
            expected_start = datetime(2025, 1, 1, 9, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
            expected_end = datetime(2025, 1, 1, 9, 55, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
...
class TestFourHourAggregator:
    ...
    async def test_get_aggregation_period(self, four_hour_aggregator):
        ...
        with patch('scripts.cron.four_hour_aggregator.datetime') as mock_datetime:
            ...
            expected_start = datetime(2025, 1, 1, 6, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
            expected_end = datetime(2025, 1, 1, 9, 55, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
...
class TestDailyAggregator:
    ...
    async def test_get_aggregation_period(self, daily_aggregator):
        ...
        with patch('scripts.cron.daily_aggregator.datetime') as mock_datetime:
            ...
            mock_now = datetime(2025, 1, 2, 10, 30, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
            mock_datetime.now.return_value = mock_now

            start_time, end_time = await daily_aggregator.get_aggregation_period()

            # 前日の期間（2025年1月1日 00:00-23:55）が返されることを確認
            expected_start = datetime(2025, 1, 1, 0, 0, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
            expected_end = datetime(2025, 1, 1, 23, 55, 0, tzinfo=pytz.timezone("Asia/Tokyo"))
