    ...
    try:
        async with async_session() as db_session:
            ...
            for condition in conditions:
                ...
                # 買いシグナルの利益計算
                for rsi, sma_20, ema_12, ema_26, sma_50, ema_50, atr, signal_price, signal_time, timeframe in buy_signals:
                ...
                # 売りシグナルの利益計算
                for rsi, sma_20, ema_12, ema_26, sma_50, ema_50, atr, signal_price, signal_time, timeframe in sell_signals:
                    if rsi and sma_20 and ema_12 and ema_26 and atr and signal_price:
                        for hours in [1, 4, 24]:
                            future_time = signal_time + timedelta(hours=hours)
            ...
            print("\n🔍 3. 条件別比較分析...")
            print("=" * 140)
            print(f"{'条件名':<15} {'買い数':<6} {'売り数':<6} {'買い1時間':<12} {'売り1時間':<12} {'買い4時間':<12} {'売り4時間':<12} {'買い1日':<12} {'売り1日':<12}")
            print("=" * 140)
            
            for result in all_results:
                print(f"{result['name']:<15} {result['buy_count']:<6} {result['sell_count']:<6} {result['buy_avg_1h']:<12.2f} {result['sell_avg_1h']:<12.2f} {result['buy_avg_4h']:<12.2f} {result['sell_avg_4h']:<12.2f} {result['buy_avg_1d']:<12.2f} {result['sell_avg_1d']:<12.2f}")
