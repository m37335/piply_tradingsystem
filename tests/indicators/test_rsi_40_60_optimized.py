...
RSIを40以下/60以上に設定し、他の条件を調整してシグナルを生成
"""
...
async def test_rsi_40_60_optimized():
    """RSI 40以下/60以上で最適化テスト"""
    print("=" * 80)
    print("🔍 RSI 40以下/60以上最適化テスト")
    print("=" * 80)
    ...
    try:
        async with async_session() as db_session:
            print("\n🔍 1. RSI 40以下/60以上でのシグナル検出...")
            print("   買い条件: RSI < 40 + 価格 > SMA20 + EMA12 > EMA26")
            print("   売り条件: RSI > 60 + 価格 < SMA20 + EMA12 < EMA26")
            print("   ※ SMA20 > SMA50条件を一時的に除外")

            # RSI 40以下/60以上条件でシグナルを検出（SMA50条件を除外）
            result = await db_session.execute(
                text(
                    ...
                    AND (
                        (ti1.value < 40 AND pd.close_price > ti2.value AND ti3.value > ti4.value AND 0.01 <= ti6.value AND ti6.value <= 0.10) OR
                        (ti1.value > 60 AND pd.close_price < ti2.value AND ti3.value < ti4.value AND 0.01 <= ti6.value AND ti6.value <= 0.10)
                    )
            ...
            if len(signals) == 0:
                print("❌ シグナルなし - さらに条件を緩和してみます")

                # さらに緩和した条件でテスト
                print("\n🔍 2. さらに緩和した条件でのテスト...")
                print("   買い条件: RSI < 40 + 価格 > SMA20")
                print("   売り条件: RSI > 60 + 価格 < SMA20")
                print("   ※ EMA条件も除外")
                ...
                result = await db_session.execute(
                    text(
                        ...
                        WHERE ti1.indicator_type = 'RSI'
                        AND (
                            (ti1.value < 40 AND pd.close_price > ti2.value AND 0.01 <= ti6.value AND ti6.value <= 0.10) OR
                            (ti1.value > 60 AND pd.close_price < ti2.value AND 0.01 <= ti6.value AND ti6.value <= 0.10)
                        )
                        ORDER BY ti1.timestamp DESC
                        LIMIT 50
                        """
            ...
            if len(signals) == 0:
                print("❌ シグナルなし - RSI 40以下/60以上は非常に厳しい条件です")
                return
            ...
            print("=" * 120)
            print(
                f"{'時刻':<20} {'タイプ':<6} {'RSI':<6} {'価格':<8} {'SMA20':<8} {'SMA50':<8} {'1時間後':<10} {'4時間後':<10} {'1日後':<10} {'1時間利益':<12} {'4時間利益':<12} {'1日利益':<12}"
            )
            print("=" * 120)
            ...
            for (
                ...
                if (
                    ...
                    if not buy_condition and not sell_condition:
                        buy_condition = (
                            rsi < 40 and signal_price > sma_20 and 0.01 <= atr <= 0.10
                        )
                        sell_condition = (
                            rsi > 60 and signal_price < sma_20 and 0.01 <= atr <= 0.10
                        )
                    ...
                    price_1h = (
                        f"{future_prices.get(1, 0):.3f}"
                        if future_prices.get(1)
                        else "N/A"
                    )
                    price_4h = (
                        f"{future_prices.get(4, 0):.3f}"
                        if future_prices.get(4)
                        else "N/A"
                    )
                    price_1d = (
                        f"{future_prices.get(24, 0):.3f}"
                        if future_prices.get(24)
                        else "N/A"
                    )

                    profit_1h = (
                        f"{profits.get(1, 0):.2f}"
                        if profits.get(1) is not None
                        else "N/A"
                    )
                    profit_4h = (
                        f"{profits.get(4, 0):.2f}"
                        if profits.get(4) is not None
                        else "N/A"
                    )
                    profit_1d = (
                        f"{profits.get(24, 0):.2f}"
                        if profits.get(24) is not None
                        else "N/A"
                    )

                    print(
                        f"{time_str:<20} {signal_type:<6} {rsi_str:<6} {price_str:<8} {sma_20_str:<8} {sma_50_str:<8} {price_1h:<10} {price_4h:<10} {price_1d:<10} {profit_1h:<12} {profit_4h:<12} {profit_1d:<12}"
                    )
            ...
            if all_profits_1h:
                ...
                win_rate_1h = (
                    len([p for p in all_profits_1h if p > 0])
                    / len(all_profits_1h)
                    * 100
                )
                print(
            ...
            if all_profits_4h:
                ...
                win_rate_4h = (
                    len([p for p in all_profits_4h if p > 0])
                    / len(all_profits_4h)
                    * 100
                )
                print(
            ...
            if all_profits_1d:
                ...
                win_rate_1d = (
                    len([p for p in all_profits_1d if p > 0])
                    / len(all_profits_1d)
                    * 100
                )
                print(
            ...
            if buy_signals:
                buy_profits_1h = [
                    ...
                    if s["profits"].get(1) is not None
                ]
                buy_profits_4h = [
                    s["profits"].get(4)
                    for s in buy_signals
                    if s["profits"].get(4) is not None
                ]
                buy_profits_1d = [
                    s["profits"].get(24)
                    for s in buy_signals
                    if s["profits"].get(24) is not None
                ]
                ...
                if buy_profits_1h:
                    ...
                    print(
                        f"買い - 1時間後: 平均{avg_buy_1h:.2f}pips, 勝率{win_rate_buy_1h:.1f}%"
                    )
                ...
                if buy_profits_4h:
                    ...
                    print(
                        f"買い - 4時間後: 平均{avg_buy_4h:.2f}pips, 勝率{win_rate_buy_4h:.1f}%"
                    )
            ...
            if sell_signals:
                ...
                if sell_profits_1h:
                    ...
                    print(
                        f"売り - 1時間後: 平均{avg_sell_1h:.2f}pips, 勝率{win_rate_sell_1h:.1f}%"
                    )
                ...
                if sell_profits_4h:
                    ...
                    print(
                        f"売り - 4時間後: 平均{avg_sell_4h:.2f}pips, 勝率{win_rate_sell_4h:.1f}%"
                    )
                ...
                if sell_profits_1d:
                    ...
                    print(
                        f"売り - 1日後: 平均{avg_sell_1d:.2f}pips, 勝率{win_rate_sell_1d:.1f}%"
                    )

            print("\n🔍 4. 条件比較と推奨...")
            print("RSI条件の比較:")
            print("- RSI 45/55: 30件のシグナル、中程度の厳格さ")
            print("- RSI 40/60: 非常に少ないシグナル、高い厳格さ")
            print("- 推奨: RSI 40以下/60以上 + 他の条件を調整")

            print("\n🎯 結論:")
            print("✅ RSI 40以下/60以上は理論的に優れた条件")
            print("✅ ただし、シグナル数が極端に少ない")
            print("✅ 他の条件（SMA50等）を調整してバランスを取る必要")
...
if __name__ == "__main__":
    asyncio.run(test_rsi_40_60_optimized())
