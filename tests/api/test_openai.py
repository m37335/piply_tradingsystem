...
class OpenAIAnalyzer:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            response = await self.generate_analysis(
                "Test connection", "USD/JPY", {"rate": 147.69}, test_mode=True
            )
    ...
    async def generate_analysis(
        ...
        if test_mode:
            ...
            prompt = self._create_analysis_prompt(
                analysis_type, currency_pair, market_data
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    self.console.print(
                        f"❌ OpenAI API request failed: HTTP {response.status_code}"
                    )
    ...
    def _create_analysis_prompt(
        self, analysis_type: str, currency_pair: str, market_data: Dict[str, Any]
    ) -> str:
        """分析プロンプト作成"""
...
[bold blue]分析タイプ:[/bold blue] {type_jp}"""
        ...
        market_panel = Panel.fit(
            market_info, title="📊 Market Data", border_style="blue"
        )
        ...
        analysis_panel = Panel.fit(
            analysis_text, title=f"🤖 AI {type_jp}", border_style="green"
        )
...
async def test_multiple_analysis_types(analyzer: OpenAIAnalyzer):
    ...
    for analysis_type, type_name in analysis_types:
        ...
        analysis_result = await analyzer.generate_analysis(
            analysis_type, "USD/JPY", market_data
        )

        if analysis_result:
            analyzer.display_analysis_result(
                analysis_type, "USD/JPY", analysis_result, market_data
            )
        else:
...
async def test_with_real_market_data(analyzer: OpenAIAnalyzer):
    ...
    if not alpha_vantage_key or alpha_vantage_key == "demo_key_replace_with_your_key":
        ...
        market_data = {"rate": 147.69, "change_24h": "+0.25", "volume": "High"}
    else:
        try:
    ...
    analysis_result = await analyzer.generate_analysis(
        "comprehensive", "USD/JPY", market_data
    )

    if analysis_result:
        analyzer.display_analysis_result(
            "comprehensive", "USD/JPY", analysis_result, market_data
        )
    else:
...
async def main():
    ...
    parser.add_argument(
        "--test",
        choices=["connection", "analysis", "multiple", "real", "all"],
        default="connection",
    ...
    if args.test == "connection":
        ...
        if analysis:
            analyzer.display_analysis_result(
                "technical", "USD/JPY", analysis, market_data
            )
        ...
        analysis = await analyzer.generate_analysis(
            "market_summary", "USD/JPY", market_data
        )
        if analysis:
            analyzer.display_analysis_result(
                "market_summary", "USD/JPY", analysis, market_data
            )
