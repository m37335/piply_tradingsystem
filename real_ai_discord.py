...
class RealAIDiscordReporter:
    ...
    async def generate_and_send_real_report(self, currency_pair: str = "USD/JPY"):
        ...
        self.console.print(f"🕘 日本時間: {datetime.now(self.jst).strftime('%Y-%m-%d %H:%M:%S JST')}")
        ...
        try:
            ...
            analysis_result = await self._generate_real_ai_analysis(currency_pair, market_data)
            if not analysis_result:
                self.console.print("❌ AI分析生成失敗")
                return False
            
            # Step 3: Discord に配信
            success = await self._send_real_discord_report(currency_pair, market_data, analysis_result)
            if success:
    ...
    async def _fetch_real_market_data(self, currency_pair: str) -> Optional[Dict[str, Any]]:
        """Alpha Vantage から実際の市場データを取得"""
        self.console.print("📊 Alpha Vantage から実データ取得中...")
        
        if not self.alpha_vantage_key or self.alpha_vantage_key == "demo_key_replace_with_your_key":
                ...
                if response.status_code == 200:
                    ...
                    if "Realtime Currency Exchange Rate" in data:
                        ...
                        market_data = {
                            ...
                            "from_currency": fx_data.get("1. From_Currency Code", from_curr),
    ...
    async def _generate_real_ai_analysis(self, currency_pair: str, market_data: Dict[str, Any]) -> Optional[str]:
...
"""
    ...
    async def _send_real_discord_report(self, currency_pair: str, market_data: Dict[str, Any], analysis: str) -> bool:
        """実AI分析結果をDiscordに配信"""
        ...
        discord_data = {
            ...
            "embeds": [{
                ...
                "description": analysis[:1000] + ("..." if len(analysis) > 1000 else ""),
                "color": color,
                "fields": [
                    {
                        "name": "💱 現在レート",
...
async def main():
    ...
    parser.add_argument("currency_pair", nargs="?", default="USD/JPY", help="Currency pair (default: USD/JPY)")
    ...
    # 環境変数チェック
    missing_keys = []
    if not reporter.alpha_vantage_key or reporter.alpha_vantage_key == "demo_key_replace_with_your_key":
        missing_keys.append("ALPHA_VANTAGE_API_KEY")
    if not reporter.openai_key or reporter.openai_key == "sk-replace-with-your-openai-key":
