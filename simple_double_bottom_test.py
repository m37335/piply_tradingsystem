...
def create_simple_double_bottom():
    """シンプルなダブルボトムデータを作成"""
    data = [
...
def test_simple_double_bottom():
    """シンプルなダブルボトムをテスト"""
    detector = DoubleTopBottomDetector()
    price_data = create_simple_double_bottom()

    print("シンプルなダブルボトムテストデータ:")
    print(f"データ数: {len(price_data)}")
    print(f"最小データ要件: 20")

    # データ長チェック
    if len(price_data) < 20:
        print("❌ データ不足のため、ダブルボトム検出は失敗します")
        return

    # ピーク検出
    peaks = detector._find_peaks(price_data, "low", window=2)
    print(f"\n検出されたピーク: {peaks}")
    ...
    if len(peaks) >= 2:
        ...
        # ネックライン検証
        neckline_result = detector._validate_neckline(
            price_data, recent_peaks, "bottom"
        )
        print(f"ネックライン検証結果: {neckline_result}")

    # ダブルボトム検出
    result = detector._detect_double_bottom(price_data)
    print(f"\nダブルボトム検出結果: {result}")


if __name__ == "__main__":
    test_simple_double_bottom()
