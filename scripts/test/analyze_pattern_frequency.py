...
class PatternFrequencyAnalyzer:
    ...
    async def _analyze_frequency_by_period(self, market_data: pd.DataFrame) -> Dict[str, Any]:
        """期間分割での頻度分析"""
        results = {}
                ...
                try:
                    ...
                    if result:
                        ...
                        if "timestamp" in result:
                            pattern_results["detection_dates"].append(result["timestamp"])
                        else:
                            pattern_results["detection_dates"].append(period_data['Date'].iloc[-1])
                except Exception as e:
    ...
    def _display_frequency_results(self, frequency_results: Dict[str, Any]):
        ...
        print(f"\n🎯 パターン別検出頻度（降順）:")
        print(f"{'順位':<4} {'パターン':<6} {'パターン名':<20} {'検出回数':<8} {'月平均':<8} {'平均間隔':<10}")
        print("-" * 80)
        ...
        for rank, (pattern_num, result) in enumerate(sorted_patterns, 1):
            ...
            print(f"{rank:<4} {pattern_num:<6} {pattern_name:<20} {total_detections:<8} {monthly_avg:<8.2f} {interval_str:<10}")
        ...
        # 高頻度パターン（月1回以上）
        high_freq_patterns = [
            (num, result) for num, result in sorted_patterns 
        ...
        if high_freq_patterns:
            ...
            for pattern_num, result in high_freq_patterns:
                print(f"   パターン{pattern_num}: {result['pattern_name']} - {result['total_detections']}回")
