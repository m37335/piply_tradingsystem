        ...
        for timeframe, data in test_data.items():
            ...
            if timeframe == 'D1' and 'macd' in indicators:
                ...
                if 'macd' in macd_data and 'signal' in macd_data:
                    ...
                    macd_condition = abs(current_macd) <= 0.2 and abs(current_signal) <= 0.2
            ...
            if timeframe == 'H4' and 'bollinger_bands' in indicators:
                ...
                if 'middle' in bb_data and not price_data.empty:
                    ...
                    bb_condition = abs(current_price - current_bb_middle) / current_bb_middle <= 0.005
            ...
            if timeframe == 'H1' and not price_data.empty:
                if len(price_data) >= 3:
                    ...
                    timeframe_analysis['volatility'] = {
                        ...
                        'avg_change': sum(price_changes) / len(price_changes) if len(price_changes) > 0 else 0.0,
                        'condition_met': volatility_condition
                    }
            
            # RSI攻防分析（M5のみ）
            if timeframe == 'M5' and 'rsi' in indicators and 'series' in indicators['rsi']:
                ...
                rsi_values = recent_rsi.tolist() if hasattr(recent_rsi, 'tolist') else list(recent_rsi)
                
                rsi_near_50 = all(40 <= rsi <= 60 for rsi in rsi_values)
                first_half = rsi_values[:3]
                second_half = rsi_values[-3:]
                rsi_crosses_50 = any(rsi < 50 for rsi in first_half) and any(rsi > 50 for rsi in second_half)
