...
class RealisticPatternFrequencyAnalyzer:
    ...
    async def analyze_realistic_frequency(self):
        ...
        try:
            ...
            logger.info(f"期間: {market_data['Date'].min()} 〜 {market_data['Date'].max()}")
    ...
    async def _analyze_realistic_patterns(self, market_data: pd.DataFrame) -> Dict[str, Any]:
        """現実的なパターン分析"""
        results = {}
    ...
    def _analyze_trend_reversals(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """トレンド転換パターンの現実的分析"""
        # 移動平均を使用したトレンド転換検出
        data = data.copy()
    ...
    def _analyze_divergences(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """ダイバージェンスパターンの現実的分析"""
        # RSIダイバージェンスの簡易検出
        data = data.copy()
        ...
        for i in range(20, len(data)):
            ...
            if (data['Close'].iloc[i] > data['Close'].iloc[i-5] and 
                data['RSI'].iloc[i] < data['RSI'].iloc[i-5]):
                divergences += 1
            # 価格が下降しているがRSIが上昇している（ブルリッシュダイバージェンス）
            elif (data['Close'].iloc[i] < data['Close'].iloc[i-5] and 
                  data['RSI'].iloc[i] > data['RSI'].iloc[i-5]):
                divergences += 1
    ...
    def _analyze_rsi_battles(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """RSI戦いパターンの現実的分析"""
    ...
    def _analyze_composite_signals(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """複合シグナルパターンの現実的分析"""
        ...
        for i in range(20, len(data)):
            ...
            if (data['RSI'].iloc[i] > 70 and 
                data['Close'].iloc[i] < data['SMA20'].iloc[i]):
                composite_signals += 1
            # RSIオーバーソールド + 価格が移動平均を上回る
            elif (data['RSI'].iloc[i] < 30 and 
                  data['Close'].iloc[i] > data['SMA20'].iloc[i]):
                composite_signals += 1
    ...
    def _analyze_engulfing_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """つつみ足パターンの現実的分析"""
        ...
        for i in range(1, len(data)):
            ...
            # 陽線が陰線を包む
            if (curr_close > curr_open and  # 陽線
                prev_close < prev_open and  # 前日陰線
                curr_open < prev_close and  # 今日の始値 < 前日の終値
                curr_close > prev_open):    # 今日の終値 > 前日の始値
                engulfing_patterns += 1
            # 陰線が陽線を包む
            elif (curr_close < curr_open and  # 陰線
                  prev_close > prev_open and  # 前日陽線
                  curr_open > prev_close and  # 今日の始値 > 前日の終値
                  curr_close < prev_open):    # 今日の終値 < 前日の始値
                engulfing_patterns += 1
    ...
    def _analyze_red_three_soldiers(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """赤三兵パターンの現実的分析"""
        red_three_soldiers = 0
        for i in range(3, len(data)):
            # 連続3日間の陽線
            if all(data['Close'].iloc[j] > data['Open'].iloc[j] for j in range(i-2, i+1)):
    ...
    def _analyze_marubozu_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """大陽線/大陰線パターンの現実的分析"""
        marubozu_patterns = 0
    ...
    def _analyze_double_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        ...
        for i in range(20, len(data)):
            ...
            if (data['High'].iloc[i] > data['High'].iloc[i-1] and
                ...
                abs(data['High'].iloc[i] - data['High'].iloc[i-10]) < 0.5):
                double_patterns += 1
            # ダブルボトム
            elif (data['Low'].iloc[i] < data['Low'].iloc[i-1] and
                  data['Low'].iloc[i] < data['Low'].iloc[i-2] and
                  abs(data['Low'].iloc[i] - data['Low'].iloc[i-10]) < 0.5):
                double_patterns += 1
    ...
    def _analyze_triple_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
    ...
    def _analyze_flag_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """フラッグパターンの現実的分析"""
        # 簡易的なフラッグパターン検出
        flag_patterns = 0
        for i in range(15, len(data)):
            # 上昇後の横ばい期間
            if (data['Close'].iloc[i] > data['Close'].iloc[i-15] and
                abs(data['Close'].iloc[i] - data['Close'].iloc[i-5]) < 1.0):
                flag_patterns += 1
    ...
    def _analyze_three_buddhas(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """三尊天井/逆三尊パターンの現実的分析"""
        # 簡易的な三尊パターン検出
        three_buddhas = 0
    ...
    def _analyze_wedge_patterns(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """ウェッジパターンの現実的分析"""
        # 簡易的なウェッジパターン検出
        wedge_patterns = 0
        for i in range(20, len(data)):
            ...
            # 高値と安値の収束チェック
            if (max(high_values) - min(high_values) < 2.0 and
                max(low_values) - min(low_values) < 2.0):
                wedge_patterns += 1
    ...
    def _analyze_support_resistance(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
    ...
    def _analyze_roll_reversals(self, data: pd.DataFrame, total_days: int) -> Dict[str, Any]:
        """ロールリバーサルパターンの現実的分析"""
        # 簡易的なロールリバーサル検出
        roll_reversals = 0
        for i in range(10, len(data)):
            ...
            # 長期トレンドと短期トレンドの逆転
            if (recent_trend > 0 and short_trend < 0) or (recent_trend < 0 and short_trend > 0):
                roll_reversals += 1
    ...
    def _display_realistic_results(self, frequency_results: Dict[str, Any], market_data: pd.DataFrame):
        """現実的な分析結果の表示"""
        print("\n" + "=" * 80)
        ...
        for rank, (pattern_num, result) in enumerate(sorted_patterns, 1):
            ...
            print(f"{rank:<4} {pattern_num:<6} {pattern_name:<20} {realistic_count:<8} {monthly_freq:<8.2f} {description:<30}")
        
        print("\n" + "=" * 80)
        
        # 頻度分類
        high_freq = [(num, result) for num, result in sorted_patterns if result["monthly_frequency"] >= 5]
        medium_freq = [(num, result) for num, result in sorted_patterns if 1 <= result["monthly_frequency"] < 5]
        low_freq = [(num, result) for num, result in sorted_patterns if result["monthly_frequency"] < 1]
        ...
        if high_freq:
            ...
            for pattern_num, result in high_freq:
                print(f"   パターン{pattern_num}: {result['pattern_name']} - {result['monthly_frequency']:.1f}回/月")
