...
class Pattern15V3CriteriaAdjuster:
        ...
        self.adjustment_patterns = [
            {
                "name": "現在設定",
                "adjustments": {
                    "5m": {"min_peaks": 2, "buffer_percentile": 20, "min_line_strength": 0.4, "max_angle": 45, "price_tolerance": 0.005},
                    "1h": {"min_peaks": 3, "buffer_percentile": 15, "min_line_strength": 0.6, "max_angle": 30, "price_tolerance": 0.003},
                    "1d": {"min_peaks": 4, "buffer_percentile": 10, "min_line_strength": 0.8, "max_angle": 20, "price_tolerance": 0.002}
                }
            },
            {
                "name": "緩和設定1",
                "adjustments": {
                    "5m": {"min_peaks": 2, "buffer_percentile": 25, "min_line_strength": 0.3, "max_angle": 60, "price_tolerance": 0.008},
                    "1h": {"min_peaks": 2, "buffer_percentile": 20, "min_line_strength": 0.5, "max_angle": 45, "price_tolerance": 0.005},
                    "1d": {"min_peaks": 3, "buffer_percentile": 15, "min_line_strength": 0.7, "max_angle": 30, "price_tolerance": 0.003}
                }
            },
            {
                "name": "緩和設定2",
                "adjustments": {
                    "5m": {"min_peaks": 1, "buffer_percentile": 30, "min_line_strength": 0.2, "max_angle": 75, "price_tolerance": 0.01},
                    "1h": {"min_peaks": 2, "buffer_percentile": 25, "min_line_strength": 0.4, "max_angle": 60, "price_tolerance": 0.008},
                    "1d": {"min_peaks": 2, "buffer_percentile": 20, "min_line_strength": 0.6, "max_angle": 45, "price_tolerance": 0.005}
                }
            }
        ]
    ...
    async def _test_with_adjusted_criteria(self, timeframe: str, period: Dict, adjustments: Dict) -> Dict:
        """調整された基準値でテスト"""
        try:
            # データ取得
            ...
            if detection:
                # 詳細分析
                detailed_analysis = self._analyze_detection_with_price_trend(detection, data, timeframe, period, adjustments)
                return {
                    "detected": True,
                    "detection": detection,
    ...
    def _create_custom_detector(self, timeframe: str, adjustments: Dict) -> SupportResistanceDetectorV3:
        """カスタムデテクター作成"""
    ...
    def _analyze_detection_with_price_trend(self, detection: Dict, data: pd.DataFrame, timeframe: str, period: Dict, adjustments: Dict) -> Dict:
        """価格推移を含む詳細分析"""
        try:
            analysis = {}
            ...
            # 価格推移分析
            analysis["price_trend"] = self._analyze_price_trend(data, equation, pattern_data)

            # 角度分析
            analysis["angle_analysis"] = self._analyze_angle_details(equation, data, pattern_data)
    ...
    def _analyze_price_trend(self, data: pd.DataFrame, equation: Dict, pattern_data: Dict) -> Dict:
        """価格推移の詳細分析"""
        try:
            analysis = {}
            ...
            analysis["line_analysis"] = {
                ...
                "price_deviation": {
                    "mean_deviation": float(np.mean(np.abs(np.array(close_prices) - np.array(line_prices)))),
                    "max_deviation": float(np.max(np.abs(np.array(close_prices) - np.array(line_prices))))
                }
            }
    ...
    def _analyze_angle_details(self, equation: Dict, data: pd.DataFrame, pattern_data: Dict) -> Dict:
        """角度の詳細分析"""
        try:
            analysis = {}
    ...
    def _analyze_timeframe_statistics(self, timeframe_results: Dict) -> Dict:
        ...
        try:
            stats = {
                "total_periods": len([k for k in timeframe_results.keys() if k != "statistics"]),
                "detection_count": 0,
                "detection_rate": 0.0,
                "period_detections": {},
            ...
            for period_name, result in timeframe_results.items():
                ...
                if result.get("detected", False):
                    ...
                    stats["pattern_type_distribution"][pattern_type] = stats["pattern_type_distribution"].get(pattern_type, 0) + 1
                else:
            ...
            for period_name in stats["confidence_by_period"]:
                stats["confidence_by_period"][period_name] = sum(stats["confidence_by_period"][period_name]) / len(stats["confidence_by_period"][period_name])

            for period_name in stats["strength_by_period"]:
                stats["strength_by_period"][period_name] = sum(stats["strength_by_period"][period_name]) / len(stats["strength_by_period"][period_name])

            for period_name in stats["angle_by_period"]:
                stats["angle_by_period"][period_name] = sum(stats["angle_by_period"][period_name]) / len(stats["angle_by_period"][period_name])
    ...
    def _analyze_pattern_statistics(self, pattern_results: Dict) -> Dict:
        ...
        try:
            ...
            if timeframe_performance:
                stats["best_performing_timeframe"] = max(timeframe_performance, key=timeframe_performance.get)
        ...
        try:
            ...
            for pattern_name, pattern_results in results.items():
                ...
                comparison["pattern_comparison"][pattern_name] = {
                    ...
                    "overall_detection_rate": pattern_stats.get("overall_detection_rate", 0.0),
                    "highest_confidence": pattern_stats.get("highest_confidence", 0.0),
                    "highest_strength": pattern_stats.get("highest_strength", 0.0),
                    "best_timeframe": pattern_stats.get("best_performing_timeframe", "N/A")
                }

            # 最高パフォーマンスパターンの特定
            best_pattern = max(
                comparison["pattern_comparison"].items(),
                key=lambda x: (x[1]["total_detections"], x[1]["overall_detection_rate"])
            )
            ...
            for pattern_name, pattern_data in comparison["pattern_comparison"].items():
                if pattern_name != "現在設定":
                    improvement = {
                        "detection_increase": pattern_data["total_detections"] - current_pattern["total_detections"],
                        "rate_improvement": pattern_data["overall_detection_rate"] - current_pattern["overall_detection_rate"],
                        "confidence_change": pattern_data["highest_confidence"] - current_pattern["highest_confidence"],
                        "strength_change": pattern_data["highest_strength"] - current_pattern["highest_strength"]
                    }
            ...
            for period_name, result in timeframe_data.items():
                ...
                if result.get("detected", False):
                    ...
                    if "price_statistics" in price_trend:
                        ...
                        print(f"          価格範囲: {high_stats.get('min', 0):.5f} - {high_stats.get('max', 0):.5f}")
                    ...
                    if "angle_basic" in angle_analysis:
                        ...
                        print(f"          角度カテゴリ: {angle_basic.get('angle_description', '')}")
                        print(f"          水平ライン: {'✅' if angle_basic.get('is_horizontal', False) else '❌'}")
