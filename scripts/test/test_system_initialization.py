...
from src.infrastructure.analysis.pattern_detectors import (
    TrendReversalDetector, PullbackDetector, DivergenceDetector,
    BreakoutDetector, RSIBattleDetector, CompositeSignalDetector,
    EngulfingPatternDetector, RedThreeSoldiersDetector, MarubozuDetector,
    DoubleTopBottomDetector, TripleTopBottomDetector, FlagPatternDetector,
    ThreeBuddhasDetector, WedgePatternDetector, SupportResistanceDetector,
...
class SystemInitializationTester:
    ...
    async def _test_analyzer_initialization(self) -> Dict:
        ...
        try:
            ...
            if not hasattr(analyzer, 'detectors'):
                return {
                    'success': False,
                    'error': 'detectors属性が存在しません'
                }

            if not hasattr(analyzer, 'patterns'):
                return {
                    'success': False,
                    'error': 'patterns属性が存在しません'
                }
    ...
    async def _test_detector_initialization(self) -> Dict:
        ...
        try:
            ...
            # 成功数の計算
            success_count = sum(1 for result in detector_results.values() 
        ...
        try:
            ...
            for pattern_num, pattern in patterns.items():
                try:
                    # 必須属性の確認
                    required_attrs = [
                        'pattern_number', 'name', 'description', 'priority',
                        'conditions', 'notification_title', 'notification_color',
                    ...
                    missing_timeframes = [
                        ...
                        if tf not in pattern.conditions
                    ]
            ...
            # 成功数の計算
            success_count = sum(1 for result in pattern_results.values() 
                              if result.get('success', False))
    ...
    async def _test_integration(self) -> Dict:
        ...
        try:
            ...
            # 成功数の計算
            success_count = sum(1 for result in detection_results.values() 
                              if result.get('success', False))
