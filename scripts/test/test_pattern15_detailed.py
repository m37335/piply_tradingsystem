...
class Pattern15DetailedTester:
    ...
    def _analyze_conditions(self, test_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 1. タッチポイント検出（レジスタンス）
            try:
                resistance_touch_points = self.detector._find_touch_points(
                    test_data, "resistance"
                )
                analysis["resistance_touch_points_count"] = len(resistance_touch_points)
                analysis["resistance_touch_points_sufficient"] = (
                    len(resistance_touch_points) >= self.detector.min_touch_points
                )
            ...
            # 2. タッチポイント検出（サポート）
            try:
                support_touch_points = self.detector._find_touch_points(
                    test_data, "support"
                )
                analysis["support_touch_points_count"] = len(support_touch_points)
                analysis["support_touch_points_sufficient"] = (
                    len(support_touch_points) >= self.detector.min_touch_points
                )
            except Exception as e:
                        ...
                        if analysis["resistance_strength_sufficient"]:
                            ...
                            if breakout is None:
                                ...
                                if current_price > line_price:
                                    ...
                                    ] = self.detector.breakout_threshold
                                    analysis["resistance_breakout_sufficient"] = (
                                        breakout_strength
                                        > self.detector.breakout_threshold
                                    )
                                else:
            ...
            # 5. レジスタンスライン検出（全体）
            try:
                resistance_line_result = self.detector._detect_resistance_line(
                    test_data
                )
            ...
            # 7. 最終結果
            analysis["either_pattern"] = (resistance_line_result is not None) or (
                support_line_result is not None
            )
