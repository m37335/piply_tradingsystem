...
class RealMarketDataTester:
    ...
    def _analyze_detection_results(self, detection_results: Dict[int, Any]) -> Dict[str, Any]:
        """検出結果の分析"""
        total_patterns = len(self.detectors)
        detected_patterns = len([r for r in detection_results.values() if r and "error" not in r])
        error_patterns = len([r for r in detection_results.values() if r and "error" in r])
