...
class WebhookConfigTester:
    ...
    async def test_webhook_configuration(self):
        ...
        print(
            f"通常のDiscord Webhook URL: {discord_webhook[:50]}..."
            if discord_webhook
            else "設定されていません"
        )

        # システム監視用Discord Webhook URL
        monitoring_webhook = self.config_manager.get(
            "notifications.discord_monitoring.webhook_url"
        )
        print(
            f"システム監視用Webhook URL: {monitoring_webhook[:50]}..."
        ...
        print(
            f"\n環境変数 DISCORD_WEBHOOK_URL: {env_discord[:50]}..."
            if env_discord
            else "設定されていません"
        )
        print(
            f"環境変数 DISCORD_MONITORING_WEBHOOK_URL: {env_monitoring[:50]}..."
            if env_monitoring
            else "設定されていません"
        )
        ...
        try:
            async with DiscordWebhookSender(monitoring_webhook) as sender:
                ...
                embed = {
                    ...
                    "color": 0x00FF00,  # 緑色
                    "fields": [
                        {"name": "テスト項目", "value": "Webhook設定確認", "inline": True},
        ...
        try:
            async with DiscordWebhookSender(discord_webhook) as sender:
                ...
                embed = {
                    ...
                    "color": 0x0000FF,  # 青色
                    "fields": [
                        {"name": "テスト項目", "value": "パターン検出通知", "inline": True},
                        {"name": "ステータス", "value": "✅ 成功", "inline": True},
                    ],
                    "timestamp": "2025-08-10T15:30:00.000Z",
                }
