...
class TALibTechnicalIndicatorsCalculator:
    ...
    async def calculate_all_indicators(self):
        ...
        try:
            ...
            for timeframe, config in self.timeframes.items():
                ...
                logger.info(
                    f"  ✅ {timeframe}完了: {timeframe_indicators}件"
                )
            ...
            for timeframe, result in timeframe_results.items():
                logger.info(
                    f"{timeframe}: {result['calculated']}件"
                )
    ...
    async def get_latest_indicators(self, timeframe: str = "M5", limit: int = 10):
        ...
        try:
            ...
            indicators = await self.indicator_service.get_latest_indicators_by_timeframe(
                timeframe, limit
            )
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if command == "all":
                ...
                logger.info("  python talib_technical_indicators_calculator.py timeframe M5")
                logger.info("  python talib_technical_indicators_calculator.py latest M5 10")
