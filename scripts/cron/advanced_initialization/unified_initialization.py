...
class UnifiedInitializer:
    ...
    async def calculate_technical_indicators(self):
        """EnhancedUnifiedTechnicalCalculator統合テクニカル指標計算"""
        try:
            logger.info(
                "=== EnhancedUnifiedTechnicalCalculator統合テクニカル指標計算開始 ==="
            )
            ...
            total_indicators = sum(results.values()) if isinstance(results, dict) else 0

            logger.info(
                f"🎉 EnhancedUnifiedTechnicalCalculator統合テクニカル指標計算完了: 合計{total_indicators}件"
            )
            return total_indicators

        except Exception as e:
            logger.error(
                f"❌ EnhancedUnifiedTechnicalCalculator統合テクニカル指標計算エラー: {e}"
            )
            return 0
