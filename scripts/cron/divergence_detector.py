...
class DivergenceDetector:
    ...
    def _detect_rsi_divergence(
        ...
        price_lows = self._find_peaks(
            -merged_data["low"].values
        )  # 安値を検出するため負の値を使用
    ...
    def display_divergences(self, divergences: List[DivergenceSignal]) -> None:
        ...
        bullish_divs = [d for d in divergences if d.divergence_type in [DivergenceType.BULLISH, DivergenceType.HIDDEN_BULLISH]]
        bearish_divs = [d for d in divergences if d.divergence_type in [DivergenceType.BEARISH, DivergenceType.HIDDEN_BEARISH]]
...
async def main():
    ...
    parser.add_argument(
        "--timeframe", "-tf", default="M5", help="時間足 (M5, H1, H4, D1)"
    )
