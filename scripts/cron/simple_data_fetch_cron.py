...
class SimpleDataFetchCron:
    ...
    async def initialize_database(self):
        ...
        try:
            ...
            await self.db_manager.initialize('sqlite+aiosqlite:///data/exchange_analytics.db')
        ...
        try:
            ...
            if data:
                ...
                result = await self.session.execute(
                    text('SELECT open_price, high_price, low_price, close_price FROM price_data WHERE id = :id'),
                    {'id': data.id}
                )
                row = result.fetchone()
                if row:
