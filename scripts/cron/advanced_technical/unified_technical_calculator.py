...
- 既存システムの統合（TechnicalIndicatorService, 
  TALibTechnicalIndicatorService, MultiTimeframeTechnicalIndicatorService）
- 差分更新システムとの連携
- マルチタイムフレーム対応
...
class UnifiedTechnicalCalculator:
    ...
    async def calculate_and_save_all_indicators(self, timeframe: str) -> Dict[str, int]:
        ...
        try:
            ...
            return {"total": total_count, "timeframe": timeframe, "status": "success"}

        except Exception as e:
            print(f"calculate_and_save_all_indicators エラー: {e}")
            return {"error": str(e)}
    ...
    async def calculate_rsi(self, data: pd.DataFrame, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            # 既存の RSI 計算ロジックを使用
            config = self.indicators_config["RSI"]
            rsi_values = talib.RSI(data["Close"].values, timeperiod=config["period"])
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status = {
                ...
                "database_connection": (
                    "connected" if self.session else "disconnected"
                ),
            }
