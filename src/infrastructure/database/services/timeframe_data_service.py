...
class TimeframeDataService:
    ...
    async def save_5m_data(self, price_data: PriceDataModel) -> PriceDataModel:
        ...
        """
        try:
            logger.info(f"Saving 5m data: {price_data.timestamp} - {price_data.close_price}")
    ...
    async def save_d1_data(self, price_data: PriceDataModel) -> PriceDataModel:
        ...
        """
        try:
            logger.info(f"Saving D1 data: {price_data.timestamp} - {price_data.close_price}")
    ...
    async def get_latest_data_by_timeframe(self, timeframe: str, limit: int = 1) -> List[PriceDataModel]:
        """
        特定時間軸の最新データを取得
        """
        try:
