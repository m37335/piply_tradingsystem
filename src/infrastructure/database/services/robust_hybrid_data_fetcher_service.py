...
class RobustHybridDataFetcherService:
    ...
    async def fetch_all_timeframes_robust(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            sorted_timeframes = sorted(
                self.timeframes.items(), key=lambda x: x[1]["priority"]
            )
    ...
    async def _fetch_timeframe_robust(self, timeframe: str) -> Dict:
        ...
        try:
            ...
            for fallback_method in strategy["fallback"]:
                try:
                    ...
                    if fallback_count > 0:
                        ...
                        logger.info(
                            f"   🔧 {timeframe}{fallback_method}補完成功: {fallback_count}件"
                        )
                    else:
            ...
            # 3. 合計計算
            result["total_count"] = result["direct_count"] + sum(
                result["fallback_counts"].values()
            )
    ...
    async def _execute_fallback_strategy(
        self, target_timeframe: str, method: str, min_data_points: Dict[str, int]
    ) -> int:
        """
        フォールバック戦略を実行
        ...
        try:
            if method == "from_5m":
                return await self._aggregate_from_5m(
                    target_timeframe, min_data_points.get("from_5m", 0)
                )
            elif method == "from_1h":
                return await self._aggregate_from_1h(
                    target_timeframe, min_data_points.get("from_1h", 0)
                )
            elif method == "from_4h":
                return await self._aggregate_from_4h(
                    target_timeframe, min_data_points.get("from_4h", 0)
                )
            else:
    ...
    async def _aggregate_from_5m(self, target_timeframe: str, min_points: int) -> int:
        ...
        try:
            ...
            if len(m5_data) < min_points:
                logger.warning(
                    f"⚠️ {target_timeframe}集計用5分足データ不足: {len(m5_data)}件 < {min_points}件"
                )
                return 0
            ...
            return await self._save_aggregated_data(
                aggregated_data, target_timeframe, "from_5m"
            )
    ...
    async def _aggregate_from_1h(self, target_timeframe: str, min_points: int) -> int:
        ...
        try:
            ...
            if len(h1_data) < min_points:
                logger.warning(
                    f"⚠️ {target_timeframe}集計用1時間足データ不足: {len(h1_data)}件 < {min_points}件"
                )
                return 0
            ...
            if target_timeframe == "4h":
                ...
                return await self._save_aggregated_data(
                    aggregated_data, target_timeframe, "from_1h"
                )
    ...
    async def _aggregate_from_4h(self, target_timeframe: str, min_points: int) -> int:
        ...
        try:
            ...
            if len(h4_data) < min_points:
                logger.warning(
                    f"⚠️ {target_timeframe}集計用4時間足データ不足: {len(h4_data)}件 < {min_points}件"
                )
                return 0
            ...
            if target_timeframe == "1d":
                ...
                return await self._save_aggregated_data(
                    aggregated_data, target_timeframe, "from_4h"
                )
    ...
    async def _aggregate_1h_to_4h(
        self, h1_data: List[PriceDataModel]
    ) -> List[PriceDataModel]:
        """1時間足から4時間足への集計"""
        # 実装は簡略化（実際はTimeframeAggregatorServiceに追加する必要あり）
        return []

    async def _aggregate_4h_to_1d(
        self, h4_data: List[PriceDataModel]
    ) -> List[PriceDataModel]:
        """4時間足から日足への集計"""
        # 実装は簡略化（実際はTimeframeAggregatorServiceに追加する必要あり）
        return []
    ...
    async def _save_aggregated_data(
        self, aggregated_data: List[PriceDataModel], timeframe: str, source_method: str
    ) -> int:
        """集計データを保存"""
        ...
        for data in aggregated_data:
            # データソースを更新
            data.data_source = (
                f"Yahoo Finance ({timeframe.upper()}) Aggregated ({source_method})"
            )
