...
class TimeframeConverter:
    ...
    def __init__(
        self,
        price_repo: PriceDataRepositoryImpl,
        indicator_repo: TechnicalIndicatorRepositoryImpl,
    ):
        self.price_repo = price_repo
        self.indicator_repo = indicator_repo

    async def create_multi_timeframe_data(
        self, start_date: datetime, end_date: datetime, currency_pair: str = "USD/JPY"
    ) -> Dict[str, Dict]:
        """
        マルチタイムフレームデータを作成
        ...
        try:
            ...
            logger.info(
                f"Created multi-timeframe data with {len(multi_timeframe_data)} timeframes"
            )
    ...
    async def _create_timeframe_data(
        ...
        try:
            ...
            return {"price_data": price_data, "indicators": indicator_dict}

        except Exception as e:
            logger.error(f"Error creating timeframe data for {db_timeframe}: {e}")
            return {"price_data": pd.DataFrame(), "indicators": {}}
