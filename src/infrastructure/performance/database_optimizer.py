...
class DatabaseOptimizer:
    ...
    def _load_optimization_thresholds(self) -> Dict[str, float]:
        """最適化閾値を読み込み"""
        return {
            "table_size_threshold": self.config_manager.get("performance.table_size_threshold", 1000000),
            "fragmentation_threshold": self.config_manager.get("performance.fragmentation_threshold", 30.0),
            "cache_hit_threshold": self.config_manager.get("performance.cache_hit_threshold", 80.0),
            "slow_query_threshold": self.config_manager.get("performance.slow_query_threshold", 5.0)
        }
    
    async def analyze_database_performance(self, session: AsyncSession) -> DatabaseMetrics:
        """データベースパフォーマンスを分析"""
        try:
            # テーブルサイズを取得
    ...
    async def optimize_database(self, session: AsyncSession) -> List[OptimizationResult]:
        """データベースを最適化"""
        try:
    ...
    async def _optimize_indexes(self, session: AsyncSession, 
                               metrics: DatabaseMetrics) -> Optional[OptimizationResult]:
        """インデックスを最適化"""
        try:
    ...
    async def _optimize_tables(self, session: AsyncSession, 
                              metrics: DatabaseMetrics) -> Optional[OptimizationResult]:
        """テーブルを最適化"""
        try:
            start_time = time.time()
            ...
            if metrics.fragmentation_ratio > self.optimization_thresholds["fragmentation_threshold"]:
                # VACUUMを実行（SQLiteの場合）
                ...
                # 改善度を計算
                improvement = metrics.fragmentation_ratio - after_metrics.fragmentation_ratio
    ...
    async def _optimize_queries(self, session: AsyncSession, 
                               metrics: DatabaseMetrics) -> Optional[OptimizationResult]:
        """クエリを最適化"""
        try:
            start_time = time.time()
    ...
    async def _send_optimization_report_to_discord(self, before_metrics: DatabaseMetrics,
    ...
    async def schedule_optimization(self, session: AsyncSession, 
