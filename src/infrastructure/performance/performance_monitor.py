...
class PerformanceMonitor:
    ...
    def __init__(self, config_manager: SystemConfigManager, log_manager: LogManager):
        ...
        self.monitoring_interval = self.config_manager.get(
            "performance.monitoring_interval", 60
        )
        ...
        return {
            ...
            "memory": {
                ...
                "critical": self.config_manager.get(
                    "performance.memory_critical", 95.0
                ),
            },
            "disk": {
            ...
            "response_time": {
                "warning": self.config_manager.get(
                    "performance.response_time_warning", 5.0
                ),
                "critical": self.config_manager.get(
                    "performance.response_time_critical", 10.0
                ),
            },
            "error_rate": {
                "warning": self.config_manager.get(
                    "performance.error_rate_warning", 5.0
                ),
                "critical": self.config_manager.get(
                    "performance.error_rate_critical", 10.0
                ),
            },
        }
    ...
    async def _check_memory_alert(
        self, metrics: PerformanceMetrics, current_time: float
    ):
        """メモリ使用率アラートをチェック"""
    ...
    async def _check_response_time_alert(
        self, metrics: PerformanceMetrics, current_time: float
    ):
        """レスポンスタイムアラートをチェック"""
    ...
    async def _create_alert(
        ...
        await self.log_manager.log_system_event(
            "PERFORMANCE_ALERT", message, level=severity, additional_data=asdict(alert)
        )
            ...
            if webhook_url:
                async with DiscordWebhookSender(webhook_url) as sender:
                    ...
                    embed = {
                        ...
                        "fields": [
                            {"name": "重要度", "value": alert.severity, "inline": True},
                            {
                                "name": "閾値",
        ...
        try:
            ...
            if webhook_url:
                async with DiscordWebhookSender(webhook_url) as sender:
                    ...
                    if report["alerts"]:
                        alert_text = "\n".join(
                            [f"• {k}: {v}回" for k, v in report["alerts"].items()]
                        )
                        embed["fields"].append(
                            {"name": "アラート統計", "value": alert_text, "inline": False}
                        )
