...
class Pattern15V2ProductionValidator:
    ...
    async def validate_pattern15_v2_production(self) -> Dict:
        ...
        try:
            ...
            return {"period_results": results, "statistics": statistics}
    ...
    def _analyze_statistics(self, results: Dict) -> Dict:
        ...
        try:
            ...
            for period_name, result in results.items():
                if result.get("detected", False):
                    ...
                    stats["pattern_types"][pattern_type] = (
                        stats["pattern_types"].get(pattern_type, 0) + 1
                    )
                    ...
                    stats["relation_stats"][relation] = (
                        stats["relation_stats"].get(relation, 0) + 1
                    )
            ...
            if stats["confidence_stats"]:
                stats["confidence_avg"] = sum(stats["confidence_stats"]) / len(
            ...
            if stats["strength_stats"]:
                stats["strength_avg"] = sum(stats["strength_stats"]) / len(
            ...
            if stats["angle_stats"]:
                stats["angle_avg"] = sum(stats["angle_stats"]) / len(
                    stats["angle_stats"]
                )
