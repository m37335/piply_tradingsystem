...
class Pattern15V3ProductionValidator:
    ...
    async def _validate_single_period(self, timeframe: str, period: Dict) -> Dict:
        ...
        try:
            ...
            if detection:
                # 詳細分析
                detailed_analysis = self._analyze_detection_detailed(detection, data, timeframe, period)
                return {
                    "detected": True,
                    "detection": detection,
    ...
    def _analyze_detection_detailed(self, detection: Dict, data: pd.DataFrame, timeframe: str, period: Dict) -> Dict:
        """検出結果の詳細分析"""
        try:
            analysis = {}
        ...
        try:
            ...
            for period_name, result in timeframe_results.items():
                ...
                if result.get("detected", False):
                    ...
                    stats["relation_by_period"][period_name][relation] = stats["relation_by_period"][period_name].get(relation, 0) + 1
        ...
        try:
            ...
            for timeframe, timeframe_results in results.items():
                ...
                for pattern_type, count in pattern_dist.items():
                    stats["pattern_type_summary"][pattern_type] = stats["pattern_type_summary"].get(pattern_type, 0) + count
...
async def main():
    ...
    if "error" not in db_info:
        ...
        print(f"  データ期間: {db_info.get('earliest_date', 'N/A')} - {db_info.get('latest_date', 'N/A')}")
        latest_data = db_info.get("latest_data", {})
        if latest_data.get("timestamp"):
            print(f"  最新データ: {latest_data['timestamp']} (Close: {latest_data['close']})")
