...
class DataFetchTester:
    ...
    async def test_data_fetch(
        self, currency_pair: str, timeframe: str, period: str
    ) -> Dict:
        """データ取得テスト実行"""
        ...
        try:
            ...
            # 2. データ取得テスト
            data_fetch_test = await self._test_data_fetch(
                currency_pair, timeframe, period
            )
    ...
    async def _test_api_connection(self) -> Dict:
        ...
        try:
            ...
            test_data = await self.alpha_vantage_client.get_intraday_data(
                symbol="USDJPY", interval="5min", outputsize="compact"
            )
            ...
            if test_data is not None and len(test_data) > 0:
                ...
                return {"success": False, "error": "APIからデータが取得できませんでした"}

        except Exception as e:
            logger.error(f"API接続テストでエラー: {e}")
            return {"success": False, "error": str(e)}

    async def _test_data_fetch(
        self, currency_pair: str, timeframe: str, period: str
    ) -> Dict:
        """データ取得テスト"""
        ...
        try:
            ...
            data = await self.alpha_vantage_client.get_intraday_data(
                symbol="USDJPY", interval=interval, outputsize="full"
            )
    ...
    async def _test_data_quality(self, data: List[Dict]) -> Dict:
        ...
        try:
            ...
            if missing_columns:
                return {"success": False, "error": f"必須カラムが不足しています: {missing_columns}"}
...
async def main():
    ...
    parser.add_argument(
        "--period", default="3months", help="期間 (1month, 3months, 6months)"
    )
    ...
    results = await tester.test_data_fetch(
        args.currency_pair, args.timeframe, args.period
    )
