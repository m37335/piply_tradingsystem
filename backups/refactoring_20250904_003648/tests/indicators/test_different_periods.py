    ...
    try:
        async with async_session() as db_session:
            ...
            for combo in period_combinations:
                if (
                    combo["sma"] in sma_periods
                    and combo["ema_short"] in ema_periods
                    and combo["ema_long"] in ema_periods
                ):
            ...
            print("=" * 120)
            print(
                f"{'期間名':<15} {'SMA':<8} {'EMA短':<8} {'EMA長':<8} {'買い数':<6} {'売り数':<6} {'買い1時間':<12} {'売り1時間':<12} {'買い4時間':<12} {'売り4時間':<12} {'買い1日':<12} {'売り1日':<12}"
            )
            print("=" * 120)

            for result in all_results:
                print(
                    f"{result['name']:<15} {result['sma']:<8} {result['ema_short']:<8} {result['ema_long']:<8} {result['buy_count']:<6} {result['sell_count']:<6} {result['buy_avg_1h']:<12.2f} {result['sell_avg_1h']:<12.2f} {result['buy_avg_4h']:<12.2f} {result['sell_avg_4h']:<12.2f} {result['buy_avg_1d']:<12.2f} {result['sell_avg_1d']:<12.2f}"
                )
