        ...
        async with async_session() as db_session:
            ...
            for timeframe in timeframes:
                ...
                for signal in signals[:2]:  # 最初の2個を表示
                    print(
                        f"  🚨 {signal.signal_type} - 信頼度{signal.confidence_score}% - 価格{signal.entry_price}"
                    )
            ...
            if all_signals:
                ...
                if webhook_url:
                    ...
                    if response.lower() == "y":
                        success = await notification_service.send_entry_signal(
                            test_signal
                        )
                        if success:
                            print("✅ Discord通知送信成功！")
                ...
                for value, timestamp, timeframe in relaxed_candidates:
                    ...
                    print(
                        f"  📊 RSI ({timeframe}): {value:.2f} - {status} at {timestamp}"
                    )
