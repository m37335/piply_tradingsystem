...
class ErrorRecoveryManager:
    ...
    async def handle_error(
        ...
        try:
            ...
            if current_retries >= strategy["max_retries"]:
                # 最大リトライ回数に達した場合
                await self._handle_max_retries_reached(
            ...
            for action in strategy["actions"]:
                success = await self._execute_recovery_action(
                    action, error_type, error_message, context
                )
                if success:
    ...
    async def _execute_recovery_action(
        ...
        try:
            if action == RecoveryAction.RETRY:
                ...
                return await self._send_error_notification(
                    error_type, error_message, context
                )
    ...
    async def _retry_operation(
        self, error_type: ErrorType, context: Optional[Dict] = None
    ) -> bool:
        """操作をリトライ"""
        try:
            strategy = self.recovery_strategies[error_type]
    ...
    async def _restart_system(self):
        ...
        try:
            ...
            await self.log_manager.log_system_event(
                "SYSTEM_RESTART", "システム全体を再起動しました", level="CRITICAL"
            )
    ...
    async def _send_error_notification(
        self, error_type: ErrorType, error_message: str, context: Optional[Dict] = None
    ) -> bool:
        """エラー通知を送信"""
        try:
            # Discordにエラー通知を送信
                ...
                async with DiscordWebhookSender(webhook_url) as sender:
                    embed = {
                        ...
                        "fields": [
                            ...
                            {
                                "name": "重要度",
                                "value": self.recovery_strategies[error_type][
                                    "severity"
                                ].value,
                                "inline": True,
                            },
                    ...
                    if context:
                        ...
                        embed["fields"].append(
                            {"name": "コンテキスト", "value": context_text, "inline": False}
                        )
    ...
    async def _handle_max_retries_reached(
        self, error_type: ErrorType, error_message: str, context: Optional[Dict] = None
    ):
        """最大リトライ回数に達した場合の処理"""
        try:
            ...
            # 重大なエラーの場合はシステムを停止
            if (
                self.recovery_strategies[error_type]["severity"]
                == ErrorSeverity.CRITICAL
            ):
                await self._send_error_notification(
                    error_type, f"最大リトライ回数に達しました: {error_message}", context
                )
                # システムを停止
                await self._restart_system()
