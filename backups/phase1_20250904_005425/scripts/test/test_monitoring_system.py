...
class MonitoringSystemTester:
    ...
    async def test_log_manager(self):
        ...
        try:
            ...
            ), f"Expected at least 3 entries, got {stats['total_entries']}"
            assert (
                stats["error_count"] >= 1
            ), f"Expected at least 1 error, got {stats['error_count']}"
            assert (
                stats["warning_count"] >= 1
            ), f"Expected at least 1 warning, got {stats['warning_count']}"

            # ログ検索をテスト
            search_results = await self.log_manager.search_logs("テスト", level="ERROR")
            assert (
                len(search_results) >= 1
            ), "Expected at least 1 error log in search results"

            # エラーサマリーを取得
            error_summary = await self.log_manager.get_error_summary(hours=1)
            assert (
                error_summary["total_errors"] >= 1
            ), "Expected at least 1 error in summary"
    ...
    async def test_system_monitor(self):
        ...
        try:
            ...
            assert (
                "cpu_percent" in status["monitoring_data"]
            ), "CPU metrics should be collected"
            assert (
                "memory_percent" in status["monitoring_data"]
            ), "Memory metrics should be collected"
            assert (
                "disk_percent" in status["monitoring_data"]
            ), "Disk metrics should be collected"

            # ヘルスレポートを生成
            health_report = await self.system_monitor.get_health_report()
            assert (
                "system_healthy" in health_report
            ), "Health report should contain system_healthy"
            ...
            assert (
                len(self.system_monitor.alert_history) >= 1
    ...
    async def test_monitoring_integration(self):
        ...
        try:
            ...
            assert (
                stats["total_entries"] >= 3
            ), "Expected at least 3 log entries from integration"
    ...
    async def test_short_monitoring_run(self):
        ...
        try:
            # 監視タスクを作成
            monitoring_task = asyncio.create_task(self._run_short_monitoring())
