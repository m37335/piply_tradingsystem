...
class AIDiscordIntegration:
    ...
    async def generate_and_send_ai_analysis(
        self, currency_pair: str = "USD/JPY", analysis_period: str = "1d"
    ):
    ...
    async def _generate_ai_report(
        self, currency_pair: str, analysis_period: str
    ) -> Dict[str, Any]:
        """AI分析レポートを生成"""
        print("📊 AI分析レポート生成中...")
    ...
    async def _send_discord_notification(
        self, report: Dict[str, Any], currency_pair: str
    ) -> bool:
        """Discord通知送信"""
        print("💬 Discord通知送信中...")
            ...
            "embeds": [
                {
                    ...
                    "color": color,
                    "fields": [
                        {"name": "💱 通貨ペア", "value": currency_pair, "inline": True},
                        {
                            "name": f"{confidence_emoji} 信頼度",
                            "value": f"{confidence:.1%}",
                            "inline": True,
                        },
                        {"name": "🤖 AI モデル", "value": model, "inline": True},
                        {
                            "name": "📈 分析要約",
                            "value": report.get("summary", "詳細分析をご確認ください"),
                            "inline": False,
                        },
                        {"name": "🕒 生成時刻", "value": generated_at, "inline": False},
                    ],
                    "footer": {"text": "Exchange Analytics AI Assistant"},
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    self.webhook_url, json=discord_data, timeout=10.0
                )
    ...
    async def send_test_notification(self):
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
                    self.webhook_url, json=test_data, timeout=10.0
                )
                if response.status_code in [200, 204]:
                    print("✅ テスト通知送信成功")
                    return True
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "test":
            ...
            await integration.generate_and_send_ai_analysis(
                currency_pair, analysis_period
            )

        else:
            print("❌ 無効なコマンド")
            print(
                "使用法: python ai_discord_integration.py [test|analyze] [currency_pair] [period]"
            )
    else:
        ...
        print(
            "  python ai_discord_integration.py analyze                 # USD/JPY 1日分析"
        )
        print(
            "  python ai_discord_integration.py analyze EUR/USD 1w      # EUR/USD 1週間分析"
        )
