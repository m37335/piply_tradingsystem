...
class TechnicalVisualizer:
    ...
    async def _get_technical_data(
        ...
        try:
            ...
            for item in indicator_data:
                # 指標タイプのフィルタリング
                if indicators != ["all"] and item.indicator_type not in [
    ...
    def _display_header(self, timeframe: str, days: int, data_count: int) -> None:
        ...
        panel = Panel(subtitle, title=title, border_style="blue", padding=(1, 2))
        console.print(panel)
    ...
    def _display_statistics(self, data: pd.DataFrame, indicators: List[str]) -> None:
        ...
        for indicator in indicators:
            if indicator == "all":
                ...
                indicator_upper = indicator.upper()
                if indicator_upper in data.columns:
                    self._add_statistics_row(
                        stats_table, indicator_upper, data[indicator_upper]
                    )
    ...
    def _add_statistics_row(
        self, table: Table, indicator: str, series: pd.Series
    ) -> None:
        """
        統計行をテーブルに追加
                ...
                if indicator == "all":
                    ...
                    if indicator_upper in data.columns:
                        signal = self._analyze_signal(
                            indicator_upper, latest[indicator_upper]
                        )
                        if signal:
                            signals.append(signal)
