...
class MomentumAnalyzer:
    ...
    async def analyze_momentum(
        ...
        try:
            ...
            # ストキャスティクスモメンタム
            stoch_momentum = self._analyze_stochastic_momentum(
    ...
    def _analyze_rsi_momentum(
        ...
        # 説明を生成
        description = self._generate_momentum_description(
            "RSI", current_rsi, previous_rsi, change_rate
        )
    ...
    def _analyze_macd_momentum(
        ...
        change_rate = (
            ((current_macd - previous_macd) / abs(previous_macd)) * 100
            if previous_macd != 0
            else 0
        )
        ...
        # 説明を生成
        description = self._generate_momentum_description(
            "MACD", current_macd, previous_macd, change_rate
        )
    ...
    def _analyze_stochastic_momentum(
        self, data: pd.DataFrame, timeframe: str
    ) -> List[MomentumSignal]:
        """ストキャスティクスモメンタムを分析"""
        signals = []
        ...
        # 説明を生成
        description = self._generate_momentum_description(
            "ストキャスティクス", current_stoch, previous_stoch, change_rate
        )
    ...
    def _analyze_ma_momentum(
        ...
        for ma_type in ["SMA", "EMA"]:
            ...
            # 説明を生成
            description = self._generate_momentum_description(
                ma_type, current_ma, previous_ma, change_rate
            )
    ...
    def _determine_momentum_type(
        self, velocity: float, change_rate: float
    ) -> MomentumType:
        """モメンタムタイプを決定"""
        # 変化率と速度の両方を考慮
    ...
    def _generate_momentum_description(
        self, indicator: str, current: float, previous: float, change_rate: float
    ) -> str:
        """モメンタム説明を生成"""
        if change_rate > 0:
            direction = "上昇"
    ...
    def display_momentum_analysis(
        self, results: Dict[str, List[MomentumSignal]]
    ) -> None:
        """モメンタム分析結果を表示"""
        if not results:
        ...
        for indicator, signals in results.items():
            ...
            for signal in signals:
                emoji = (
                    ...
                    if signal.momentum_type
                    in [MomentumType.STRONG_DOWN, MomentumType.DOWN]
                    else "⚪"
                )
        ...
        console.print(
            f"  {emoji} {overall_momentum} (上昇: {up_signals}件, 下降: {down_signals}件)"
        )
