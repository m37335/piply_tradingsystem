...
class AdvancedSignalAnalyzer:
    ...
    async def analyze_comprehensive_signals(
    ...
    async def _analyze_current_signals(
    ...
    def _analyze_stochastic_signal(
        self, stoch_value: float
    ...
    async def _generate_summary(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 平均信頼度
        avg_confidence = (
            np.mean([s.confidence for s in current_signals]) if current_signals else 0
        )
        ...
        if "current_signals" in results:
            ...
            if signals:
                ...
                for signal in signals:
                    ...
                    console.print(
                        f"  {emoji} {signal.signal_type.value} ({signal.strength.value}) - {signal.reasoning}"
                    )
            ...
            if trends:
                ...
                for indicator, trend in trends.items():
                    if "error" not in trend:
                        console.print(
                            f"  {indicator}: {trend['direction']} (強度: {trend['strength']:.2f})"
                        )
        ...
        if "volatility_analysis" in results:
            ...
            if volatility:
                ...
                for indicator, vol in volatility.items():
                    console.print(
                        f"  {indicator}: {vol['level']} (現在: {vol['current']:.3f})"
                    )
...
async def main():
    ...
    try:
        # 分析実行
        results = await analyzer.analyze_comprehensive_signals(
